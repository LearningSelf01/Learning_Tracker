...
class _TeacherSignUpPageState extends State<TeacherSignUpPage> {
  ...
  Future<void> _startPhoneVerification(String phone) async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(const SnackBar(content: Text('Could not start phone verification.')));
    }
  }
                    ...
                    : () async {
                        ...
                        try {
                          ...
                          if (mounted) {
                            ScaffoldMessenger.of(context)
                              ..hideCurrentSnackBar()
                              ..showSnackBar(const SnackBar(content: Text('Phone verified successfully.')));
                          }
                          ...
                          if (mounted) {
                            ScaffoldMessenger.of(context)
                              ..hideCurrentSnackBar()
                              ..showSnackBar(SnackBar(content: Text('Invalid code: ${e.message}')));
                          }
                        } finally {
  ...
  Future<void> _resendSmsOtp(String phone) async {
    try {
      ...
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(SnackBar(content: Text('OTP resent to $phone')));
    } on AuthException catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(SnackBar(content: Text('Could not resend code: ${e.message}')));
    } catch (_) {
      if (!mounted) return;
      ScaffoldMessenger.of(context)
        ..hideCurrentSnackBar()
        ..showSnackBar(const SnackBar(content: Text('Unexpected error while resending code.')));
    }
  }
